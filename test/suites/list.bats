...
@test 'list all castles' {
  ...
  esc="\\u001b\\u005b"
  cat <<EOF | expect -f -
      spawn "$HOMESHICK_DIR/bin/homeshick" list
...
@test 'list castle with altered upstream remote name' {
  ...
  castle 'rc-files'
  (cd "$HOME/.homesick/repos/rc-files" && git remote rename origin nigiro)
  esc="\\u001b\\u005b"
  cat <<EOF | expect -f -
      spawn "$HOMESHICK_DIR/bin/homeshick" list
...
@test 'list castle with slash in branchname' {
  ...
  castle 'rc-files'
  (cd "$HOME/.homesick/repos/rc-files" && git checkout branch/with/slash)
  esc="\\u001b\\u005b"
  cat <<EOF | expect -f -
      spawn "$HOMESHICK_DIR/bin/homeshick" list
      expect -ex "${esc}1;37m     rc-files${esc}0m $REPO_FIXTURES/rc-files\r\n" {} default {exit 1}
